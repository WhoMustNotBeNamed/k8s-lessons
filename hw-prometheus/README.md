...
### 2. Как проверить работоспособность всех компонентов системы
   ...
   - Проверьте, что Grafana отображает дашборды с метриками, перейдя по ссылке:
...
Для получения метрик и создания тестовой нагрузки необходимо запустить Jmeter скрипт командой:
```bash
